...
def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'lambda':
        def lambda_rule(epoch):
            ...
            lr_l = 1e-6 if lr_l < 1e-6 else lr_l
            return lr_l
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    elif which_model_netG == 'UNext':
        netG = UNext(num_classes=1, img_size=opt.fineSize)
        init_flag = False

    elif which_model_netG == 'Tokenized_ResNet_0_9':
        netG = Tokenized_ResNet(input_nc, num_features=64, num_residuals=9, out_channels=1, img_size=512, opt=opt, token_channel=opt.token_channel,
...
class GANLoss(nn.Module):
    ...
    def __call__(self, input, target_is_real):
        if not isinstance(input, tuple):
            ...
            loss = 0
            for fake_input in input:
                target_tensor = self.get_target_tensor(fake_input, target_is_real)
                loss += self.loss(fake_input, target_tensor)
            return loss
