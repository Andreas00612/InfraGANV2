...
from models.CCPL_function import calc_mean_std, nor_mean_std, nor_mean, calc_cov
import random
...
class Net(nn.Module):
    def __init__(self):
        ...
        decoder = nn.Sequential(*list(decoder1.children())[10:])
        encoder = vgg16()
        #encoder = tv.vgg16(pretrained=True).features
    ...
    def encode_with_intermediate(self, input):
        ...
        for i in range(self.end_layer):
            func = getattr(self, 'enc_{:d}'.format(i + 1)).cuda()
    ...
    def forward(self, content, style, num_layer):
        ...
        loss_ccp = self.CCPL(g_t_feats, content_feats, start_layer, end_layer)
