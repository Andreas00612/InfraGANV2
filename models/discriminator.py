...
from models.wavelet import get_wav, WaveUnpool, WavePool
...
class SesameNLayerDiscriminator(nn.Module):
    ...
    def __init__(self, opt, input_nc=None, nf=64, kw=4):
        ...
        if opt.wavelet_disc:
            self.img_pool = WavePool(in_channels=3, out_channels=3).cuda()
            self.infra_pool = WavePool(in_channels=1, out_channels=1).cuda()
...
class Wavelet_Sesame_Disc(nn.Module):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        parser.add_argument('--n_layers_D', type=int, default=4,
                            help='# layers in each discriminator')
    ...
    def __init__(self, opt, input_nc=None, nf=64, kw=4):
        ...
        wave_nf = 128
        semWave_sequence = [[nn.Conv2d(4, wave_nf, kernel_size=kw, stride=2, padding=padw),
                             nn.LeakyReLU(0.2, False)]]
        norm_layer = get_nonspade_norm_layer(opt, 'spectralinstance')
        for n in range(1, opt.n_layers_D - 1):
            nf_prev = wave_nf
            wave_nf = min(wave_nf * 2, 512)
            stride = 1 if n == opt.n_layers_D - 2 else 2
            semWave_sequence += [[norm_layer(nn.Conv2d(nf_prev, wave_nf, kernel_size=kw,
                                                       stride=stride, padding=padw)),
                                  nn.LeakyReLU(0.2, False)
                                  ]]

        self.wave_sequence = nn.ModuleList()
        for n in range(len(semWave_sequence)):
            self.wave_sequence.append(nn.Sequential(*semWave_sequence[n]))
        self.wave_sequence = nn.Sequential(*self.wave_sequence)
    ...
    def forward(self, input):
        """input:rgb_img sem:infra_img"""
        img, sem = input[:, -3:], input[:, :-3]

        sem_wavelet = self.infra_pool(sem)
        sem_wavelet = torch.cat(sem_wavelet, dim=1)
        sem_wavelet_result = self.wave_sequence(sem_wavelet)
